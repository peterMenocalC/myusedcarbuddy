...
import 'package:flutter_app_mucb/Fragments/HomePage.dart';
import 'package:flutter_app_mucb/RefreshCallback.dart';
...
class assignedPage extends StatefulWidget {
  static const String routeName = '/assignedPage';
  final name;
  final userId;

  assignedPage({Key key, @required this.name, @required this.userId})
      : super(key: key);
...
class _assignedPageState extends State<assignedPage>
    implements RefreshDataCallback {
  ...
  Future getRequestListData() async {
    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await fetchRequestListData(widget.userId, "2").then((res) async {
          setState(() {
            mIsLoading = false;
        ...
        child: Scaffold(
            ...
            body: new Stack(
              children: [
                ...
                Visibility(
                    visible: isErrorOccured,
                    child: createErrorView(context, "No task assigned", this)),
                mIsLoading
                    ? Container(
...
class SelectCard extends StatelessWidget {
  const SelectCard({Key key, this.report, @required this.userId})
      : super(key: key);
