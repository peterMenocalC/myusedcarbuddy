...
import 'package:flutter_app_mucb/ScheduledInspectionDetails.dart';
...
class scheduledPage extends StatefulWidget {
  static const String routeName = '/scheduledPage';
  final name;
  final userId;

  scheduledPage({Key key, @required this.name, @required this.userId})
      : super(key: key);
...
class _scheduledPageState extends State<scheduledPage> implements RefreshDataCallback {
  ...
  Future getRequestListData() async {
    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await fetchRequestListData(widget.userId, "3").then((res) async {
          setState(() {
            mIsLoading = false;
        ...
        child: new Scaffold(
            ...
            Visibility(visible: !isErrorOccured,  child: Padding(
                ...
                Visibility(
                    ...
                    child: createErrorView(context, "No task scheduled", this)),
                mIsLoading
                    ? Container(
                    ...
                    InkWell(
                        onTap: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (_) => ScheduleInspectionDetails(
                                    userId: userId,
